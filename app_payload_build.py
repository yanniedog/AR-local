...
def _attach_terms(data, terms_root):
    if terms_root is not None:
        from app_payload_terms import load_published_terms
        data['terms'] = load_published_terms(
            terms_root, source_observation=data.get('source_observation'),
            run_date=data['run_date'], product_keys=data['details']['products'])
...
def _package(
    ...
    *,
    ...
    if terms:
        if enc_key:
            raise ValueError('Terms shards require a consumer-compatible unencrypted evidence channel')
    ...
    if executable_v2_root is not None:
        if enc_key:
            raise ValueError('Eligibility v2 requires an unencrypted verified channel')
        from app_payload_executable_v2 import load_published_executable_v2, package_executable_v2
        snapshot = load_published_executable_v2(executable_v2_root, source_observation=source_observation,
            run_date=run_date, core_asset_sha256=files['core']['sha256'], details_asset_sha256=files['details']['sha256'],
            product_keys=details.get('products', {}))
        manifest['executable_v2'] = package_executable_v2(snapshot, core=core, details=details,
            core_asset_sha256=files['core']['sha256'], details_asset_sha256=files['details']['sha256'], run_date=run_date,
            write_asset=lambda kind, value: _asset(out_dir, kind, run_date, _gzip_bytes(value), release_base))
    if executable_v3_root is not None:
        if enc_key:raise ValueError('Monetary v3 requires an unencrypted verified channel')
        from app_payload_executable_v3 import load_published_executable_v3,package_executable_v3
        snapshot=load_published_executable_v3(executable_v3_root,source_observation=source_observation,
        ...
        namespace=package_executable_v3(snapshot,core=core,details=details,core_asset_sha256=files['core']['sha256'],
