-- DESIGN DRAFT ONLY. NOT EXECUTED. Controller owns BEGIN IMMEDIATE and marker insertion.
-- Baseline 001 is immutable; do not rebuild, rename, copy or modify v1/v2 tables.
-- foreign_keys must already be ON before BEGIN. Run statements individually, never executescript.
CREATE TABLE executable_registry_migrations_v3 (
 migration_id TEXT PRIMARY KEY CHECK(migration_id='002_monetary_v3'),
 predecessor_migration_id TEXT NOT NULL REFERENCES executable_registry_migrations(migration_id) CHECK(predecessor_migration_id='001_scoped_eligibility_v2'),
...
CREATE TABLE executable_scopes_v3 (
 scope_id TEXT PRIMARY KEY CHECK(length(scope_id)=64),
 capability TEXT NOT NULL CHECK(capability IN ('savings_calculation','mortgage_calculation','portfolio_calculation')),
...
CREATE INDEX executable_scopes_v3_product ON executable_scopes_v3(product_key,capability,scope_id);
CREATE TABLE executable_subjects_v3 (
 ...
 adapter_version TEXT NOT NULL, evaluator_version TEXT NOT NULL,
 ...
 FOREIGN KEY(scope_id,capability) REFERENCES executable_scopes_v3(scope_id,capability),
 FOREIGN KEY(previous_subject_id,scope_id) REFERENCES executable_subjects_v3(subject_id,scope_id),
...
CREATE INDEX executable_subjects_v3_slot ON executable_subjects_v3(scope_id,sequence DESC);
CREATE INDEX executable_subjects_v3_current ON executable_subjects_v3(observation_id,capability,scope_id,sequence DESC);
CREATE TRIGGER executable_subjects_v3_scope_source BEFORE INSERT ON executable_subjects_v3
WHEN NOT EXISTS(
 SELECT 1 FROM executable_scopes_v3 k JOIN observations o ON o.observation_id=NEW.observation_id
...
CREATE TABLE executable_subject_terms_v3 (
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id),
 term_revision_id TEXT NOT NULL REFERENCES term_revisions(term_revision_id),
 PRIMARY KEY(subject_id,term_revision_id)
);
CREATE TABLE executable_subject_documents_v3 (
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id),
...
CREATE TABLE executable_authorities_v3 (
...
CREATE TABLE executable_subject_authorities_v3 (
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id),
 authority_id TEXT NOT NULL REFERENCES executable_authorities_v3(authority_id),
 PRIMARY KEY(subject_id,authority_id)
);
CREATE INDEX executable_subject_authorities_v3_reverse ON executable_subject_authorities_v3(authority_id,subject_id);
CREATE TABLE executable_subject_observations_v3 (
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id),
 observation_id TEXT NOT NULL REFERENCES observations(observation_id),
 PRIMARY KEY(subject_id,observation_id)
);
CREATE INDEX executable_subject_observations_v3_reverse ON executable_subject_observations_v3(observation_id,subject_id);
CREATE TABLE executable_subject_members_v3 (
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id),
...
CREATE INDEX executable_subject_members_v3_reverse ON executable_subject_members_v3(member_sha256,subject_id);
CREATE INDEX executable_subject_terms_v3_reverse ON executable_subject_terms_v3(term_revision_id,subject_id);
CREATE INDEX executable_subject_documents_v3_reverse ON executable_subject_documents_v3(document_version_id,subject_id);
CREATE TRIGGER immutable_executable_authorities_v3_UPDATE BEFORE UPDATE ON executable_authorities_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_authorities_v3_DELETE BEFORE DELETE ON executable_authorities_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_authorities_v3_UPDATE BEFORE UPDATE ON executable_subject_authorities_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_authorities_v3_DELETE BEFORE DELETE ON executable_subject_authorities_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_observations_v3_UPDATE BEFORE UPDATE ON executable_subject_observations_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_observations_v3_DELETE BEFORE DELETE ON executable_subject_observations_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_members_v3_UPDATE BEFORE UPDATE ON executable_subject_members_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
CREATE TRIGGER immutable_executable_subject_members_v3_DELETE BEFORE DELETE ON executable_subject_members_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
...
CREATE TABLE executable_reviews_v3 (
 sequence INTEGER PRIMARY KEY AUTOINCREMENT,
 review_id TEXT NOT NULL UNIQUE CHECK(length(review_id)=64),
 subject_id TEXT NOT NULL REFERENCES executable_subjects_v3(subject_id), previous_review_id TEXT,
 ...
 FOREIGN KEY(previous_review_id,subject_id) REFERENCES executable_reviews_v3(review_id,subject_id),
...
CREATE INDEX executable_reviews_v3_subject ON executable_reviews_v3(subject_id,sequence DESC);
CREATE TABLE executable_publications_v3 (
 sequence INTEGER PRIMARY KEY AUTOINCREMENT,
 ...
 FOREIGN KEY(previous_publication_id,product_key,capability)
  REFERENCES executable_publications_v3(publication_id,product_key,capability),
 ...
 CHECK(state='removed' OR json_extract(payload_json,'$.schemaVersion') IS 3),
...
CREATE INDEX executable_publications_v3_product ON executable_publications_v3(product_key,capability,sequence DESC);
CREATE TRIGGER executable_publications_v3_source BEFORE INSERT ON executable_publications_v3
...
CREATE TRIGGER executable_subjects_v3_cas BEFORE INSERT ON executable_subjects_v3
WHEN NEW.previous_subject_id IS NOT (SELECT subject_id FROM executable_subjects_v3 WHERE scope_id=NEW.scope_id ORDER BY sequence DESC LIMIT 1)
BEGIN SELECT RAISE(ABORT,'stale monetary predecessor'); END;

CREATE TRIGGER executable_reviews_v3_cas BEFORE INSERT ON executable_reviews_v3
WHEN NEW.previous_review_id IS NOT (SELECT review_id FROM executable_reviews_v3 WHERE subject_id=NEW.subject_id ORDER BY sequence DESC LIMIT 1)
BEGIN SELECT RAISE(ABORT,'stale monetary predecessor'); END;

CREATE TRIGGER executable_publications_v3_cas BEFORE INSERT ON executable_publications_v3
WHEN NEW.previous_publication_id IS NOT (SELECT publication_id FROM executable_publications_v3 WHERE product_key=NEW.product_key AND capability=NEW.capability ORDER BY sequence DESC LIMIT 1)
BEGIN SELECT RAISE(ABORT,'stale monetary predecessor'); END;

CREATE TRIGGER executable_subjects_v3_no_executable_templates_collision BEFORE INSERT ON executable_subjects_v3
...
CREATE TRIGGER executable_templates_no_executable_subjects_v3_collision BEFORE INSERT ON executable_templates
WHEN EXISTS(SELECT 1 FROM executable_subjects_v3 WHERE subject_id=NEW.template_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable subject identity collision'); END;

CREATE TRIGGER executable_subjects_v3_no_executable_subjects_v2_collision BEFORE INSERT ON executable_subjects_v3
WHEN EXISTS(SELECT 1 FROM executable_subjects_v2 WHERE subject_id=NEW.subject_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable subject identity collision'); END;

CREATE TRIGGER executable_subjects_v2_no_executable_subjects_v3_collision BEFORE INSERT ON executable_subjects_v2
WHEN EXISTS(SELECT 1 FROM executable_subjects_v3 WHERE subject_id=NEW.subject_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable subject identity collision'); END;

CREATE TRIGGER executable_reviews_v3_no_executable_reviews_collision BEFORE INSERT ON executable_reviews_v3
...
CREATE TRIGGER executable_reviews_no_executable_reviews_v3_collision BEFORE INSERT ON executable_reviews
WHEN EXISTS(SELECT 1 FROM executable_reviews_v3 WHERE review_id=NEW.review_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable review identity collision'); END;

CREATE TRIGGER executable_reviews_v3_no_executable_reviews_v2_collision BEFORE INSERT ON executable_reviews_v3
WHEN EXISTS(SELECT 1 FROM executable_reviews_v2 WHERE review_id=NEW.review_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable review identity collision'); END;

CREATE TRIGGER executable_reviews_v2_no_executable_reviews_v3_collision BEFORE INSERT ON executable_reviews_v2
WHEN EXISTS(SELECT 1 FROM executable_reviews_v3 WHERE review_id=NEW.review_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable review identity collision'); END;

CREATE TRIGGER executable_publications_v3_no_executable_publications_collision BEFORE INSERT ON executable_publications_v3
...
CREATE TRIGGER executable_publications_no_executable_publications_v3_collision BEFORE INSERT ON executable_publications
WHEN EXISTS(SELECT 1 FROM executable_publications_v3 WHERE publication_id=NEW.publication_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable publication identity collision'); END;

CREATE TRIGGER executable_publications_v3_no_executable_publications_v2_collision BEFORE INSERT ON executable_publications_v3
WHEN EXISTS(SELECT 1 FROM executable_publications_v2 WHERE publication_id=NEW.publication_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable publication identity collision'); END;

CREATE TRIGGER executable_publications_v2_no_executable_publications_v3_collision BEFORE INSERT ON executable_publications_v2
WHEN EXISTS(SELECT 1 FROM executable_publications_v3 WHERE publication_id=NEW.publication_id)
BEGIN SELECT RAISE(ABORT,'cross-version executable publication identity collision'); END;

CREATE TRIGGER immutable_executable_registry_migrations_v3_UPDATE BEFORE UPDATE ON executable_registry_migrations_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_registry_migrations_v3_DELETE BEFORE DELETE ON executable_registry_migrations_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_scopes_v3_UPDATE BEFORE UPDATE ON executable_scopes_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_scopes_v3_DELETE BEFORE DELETE ON executable_scopes_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subjects_v3_UPDATE BEFORE UPDATE ON executable_subjects_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subjects_v3_DELETE BEFORE DELETE ON executable_subjects_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subject_terms_v3_UPDATE BEFORE UPDATE ON executable_subject_terms_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subject_terms_v3_DELETE BEFORE DELETE ON executable_subject_terms_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subject_documents_v3_UPDATE BEFORE UPDATE ON executable_subject_documents_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_subject_documents_v3_DELETE BEFORE DELETE ON executable_subject_documents_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_reviews_v3_UPDATE BEFORE UPDATE ON executable_reviews_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_reviews_v3_DELETE BEFORE DELETE ON executable_reviews_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_publications_v3_UPDATE BEFORE UPDATE ON executable_publications_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;

CREATE TRIGGER immutable_executable_publications_v3_DELETE BEFORE DELETE ON executable_publications_v3
BEGIN SELECT RAISE(ABORT,'terms evidence is append-only'); END;
