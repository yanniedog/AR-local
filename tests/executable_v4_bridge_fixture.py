...
from cdr_terms.executable_v4_oracle import evaluate as financial_expectation

ROOT=Path(__file__).parent/'fixtures/activity-v4'


@pytest.fixture
def actual_activity_bridge(tmp_path):
    bridge=json.loads(gzip.decompress((ROOT/'actual-bridge.json.gz').read_bytes()));codes=json.loads((ROOT/'code-identities.json').read_bytes())
...
def retain_benchmark(store,bridge,codes):
    ...
    run=dict(schemaVersion=4,subjectId=subject['id'],capability=subject['capability'],adapterVersion=subject['adapterVersion'],evaluatorVersion=subject['evaluatorVersion'],executionActor='retained actual app adapter execution',suiteSha256=suite,contextSha256=context,cases=cases)
