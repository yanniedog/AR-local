...
#   1. ensure the status API is running,
...
# 1. Status API up? (Restart=always should handle it, but make boot deterministic.)
if ! systemctl is-active --quiet ar-local-status.service; then
  log "status API not active; starting"
  sudo systemctl start --no-block ar-local-status.service || log "WARN: status API start failed"
fi
...
if [ -f "$REPO_DIR/pi_ingest_alert.py" ]; then
  ...
  /usr/bin/python3 "$REPO_DIR/pi_ingest_alert.py" \
    ...
    --details "Pi booted/rebooted ($uptime_str). Boot-recovery ran status check + ingest catch-up." \
