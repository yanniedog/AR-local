...
Updated 2026-09-15 for the operator's approved plan. The current evidence table
...
## Current evidence and limits (2026-09-15)
...
| Current bank/product/field reporting | PASS, report boundary only | `runs/product-audit-20260914/reports/current-006/audit-evidence.json` and CSV/JSON/HTML companions: 110 named banks, 2,824 products, 16,681 rates, 286,770 current detail parameters and 3,052,274 historical parameter baseline/change rows. The selected index has 124 dates and two missing calendar dates; its latest bundle matches the report. The retained metric dictionary explicitly describes the September 14 baseline. Current source/public deltas of 157 rates and four features remain unreconciled. Full fine print, unknown source denominators, visual QA and executable product semantics remain gaps. |
...
| Declarative eligibility and dated exact ledger | PASS, supported helper tests only | AR-app commit `9dc52059f3fbaa000fd3d4398bd92b6cbf25642d`: 67 focused engine/runtime tests cover exact arithmetic, source-quote hashes, clause benchmarks, dated additive savings components, marginal/whole-balance tiers, contract-owned assessment windows, explicit account sets and three-valued activity qualification. Evaluator v2 is version-bound; older consumers reject unsupported contracts. No full-product rule adapter is enabled; mortgage principal allocation, full savings/TD policies and material fee coverage remain unfinished. |
| Candidate AR-app CI | PASS, local candidate code/packaging only; native BLOCKED_EXTERNAL | Candidate `9dc52059f3fbaa000fd3d4398bd92b6cbf25642d`: full local `mobile/npm run ci` passed 172 Jest suites / 1,677 tests, 120 script tests, doctor, typecheck, lint, exports and size guards. Private production-signed APK 1.0.204/code267: 87,803,753 bytes, SHA-256 `1a22f7703bb739cedd5d66b7dd70fefdac9b2a7946aff752add53eb8d8202dc6`; native/embedded versions and all four ABIs verified against the clean exact head. It is 50,832 bytes above released 1.0.203. Native execution, installation and publication were not performed; this is not deployed-consumer proof. |
| Encrypted customer profile (C-003) | IN_PROGRESS, code tests only | Typed on-device answers preserve unknown/unavailable/not-applicable and explicit zero/false, conservative legacy migration, source/effective provenance, scoped questions and negotiated terms. SecureStore interruption and editor draft preservation are covered by 34 focused tests. Real-device persistence, accessibility and full product adapters remain unverified. |
| Canonical parameter registry (T-004) | IN_PROGRESS | PR758 introduced 13 typed descriptive parameters and pinned job context. Three late findings concerning ambiguous aliases, admission of new legacy contexts and deterministic rejection retries are being corrected; this is not a complete reviewed registry or executable rule catalogue. |
| May 23 historical fee canary | FAIL, retained terminal evidence | The approved one-shot attempt exhausted its 600-second archive deadline; no candidate or composition receipt was produced. The permanent claim remains consumed and full reservation retained. PR757/759 checkpoint and Windows checkout fixes passed code tests but do not authorize a source replay or prove a successful canary. |
...
| Critical-window supervision | PASS for local schedule configuration; live protection BLOCKED_EXTERNAL | The existing automation was paused, prior-owner safe-idle acknowledged, then retargeted to the current owner and reactivated with its schedule preserved. No duplicate was created. Windows/Codex availability and authenticated Pi access remain dependencies; missed or late checkpoints are not successful on-time preflights. The 00:30 quiet window and ingest/reclaim protections remain binding. |
| Existing Pi dashboard baseline | Latest smoke FAIL; current deployment acceptance unverified | An earlier pre-deployment HTTP smoke passed, but the latest `npm run verify:pi` failed on the mortgage raw-history 30-second timeout. External Chrome observed the live 90-day history window, which does not establish loss of older published assets. No retry, restart or candidate deployment is implied by the older pass. |
...
- [ ] **C-003 IN_PROGRESS** Provide dynamic encrypted on-device customer profiles
...
- [ ] **C-007 IN_PROGRESS** Savings scenarios cover tier boundaries, base/bonus/
