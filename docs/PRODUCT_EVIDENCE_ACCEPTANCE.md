...
| Incorporated-document graph (D-002) | IN_PROGRESS, bounded local implementation | Append-only root/scope/node/edge/expansion records preserve exact raw references, parent versions, extraction anchors and shared URL identities. One retained-node expansion plus at most one fetch uses existing worker guards; explicit host grants, redirect/304 entity binding, bounds, cycles, restart and stale-source tests use retained CDR fixtures. Graph-only links cannot create reviewed applicability or interpretation jobs. Full PDF/OCR/reference closure, runtime proof and daily acquisition capacity remain unfinished. |
