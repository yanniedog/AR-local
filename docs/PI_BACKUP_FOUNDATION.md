...
After an authorized deployment passes exact-SHA synchronization, service checks, and the dashboard smoke test, `pi_deploy_verify.py` invokes the candidate's `record-deployment` command. It writes a create-once acceptance record on the external backup filesystem, bound to the protected and candidate SHAs, plan identities, exact parent and remote commands, backup/restore/boot evidence hashes, verification results, operator, timestamps, deviations, and the prior deployment-record digest. Failure to create this record makes the deployment command fail; a successful deployment is never represented only by console output.
