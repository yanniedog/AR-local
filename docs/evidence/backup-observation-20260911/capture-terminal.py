...
def main():
    ...
    before = task_state()
    if not before['last_run'].startswith('2026-09-11T06:00:'):
        raise ValueError('unexpected observed task execution')
    ...
    completed = [g for g in guards if g['mode'] == 'run' and g['started_at_utc'].startswith('2026-09-10T20:00:')]
    if len(completed) != 1:
    ...
    result = {'at_utc': datetime.now(timezone.utc).isoformat(), 'task': before,
              ...
              'origin': 'UNVERIFIED: 06:00 task execution observed; TaskScheduler Operational history disabled', 'natural_trigger': 'UNVERIFIED', 'physical_recovery': 'BLOCKED',
