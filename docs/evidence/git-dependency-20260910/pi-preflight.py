...
assert hashlib.sha256(reader.read_bytes()).hexdigest() == '2c8b264af5f6f0dc52026f7d5e03e8c2aec8ec63f30f57f848c8f59e8841c1a7'
runtime = runpy.run_path(str(reader))
path = Path(r'C:\code\backups\AR-local-user-session\private-tools-20260910-v2\user-session-backup.json')
assert hashlib.sha256(path.read_bytes()).hexdigest() == '1b46d6a1db222602bbb5217210f9a5f0fb6e750d82b801b55862004cc2d5ae97'
...
out = Path(__file__).parent / ('pi-preflight-' + started.strftime('%Y%m%dT%H%M%S') + '.json')
