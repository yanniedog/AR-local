...
binding_path = output / 'binding/receipt-binding.json'
...
task = dict(capture['task'],name='AR-local user-session backup',
            action_changed=True,principal_triggers_settings_preserved=True,
...
summary = {'task':task,'guard':guard,'selected_generation':selected,
    'backup':{'result':'PASS' if passed else 'FAIL','scheduled_result':scheduled['result'],
        ...
        'natural_trigger':False,'physical_recovery':'BLOCKED',
        'decision':'D-023-PRIVATE-BACKUP-TOOLS-20260910; pre-action intent and manual-start intent preserved'},
    ...
    'scope':'Actual manual operational outcome and frozen evidence; A3 natural and A4 physical acceptance remain open'}
