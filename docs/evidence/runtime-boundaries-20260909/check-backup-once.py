...
attempt = Path(__file__).with_name("check-only-authority.json")
with attempt.open("x", encoding="utf-8") as stream:
    json.dump({"user_instruction": "Do it now rather than 6am.",
               ...
               "scope": "Check-only September 9; no backup-data copy; one-connection cleanup; SSH ConnectTimeout=60 seconds; ordinary token, identity, locks, quiet-window and receiver preflight retained",
...
code = runner.execute(config, "check", CONFIG_SHA)
