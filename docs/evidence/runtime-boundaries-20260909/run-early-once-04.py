...
attempt = Path(__file__).with_name("early-start-authority-04.json")
with attempt.open("x", encoding="utf-8") as stream:
    json.dump({"user_instruction": "Do it now rather than 6am.",
               ...
               "scope": "One invocation before 06:00 September 9; SSH ConnectTimeout=25 seconds; ordinary token, identity, locks, quiet-window and receiver preflight retained",
...
def tolerant_ssh_options(*args, **kwargs):
    ...
    return ["ConnectTimeout=25" if item == "ConnectTimeout=10" else item for item in options]
