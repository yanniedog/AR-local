...
GIB = 1024**3
SCHEMA = "ar-local-quality-resources-v1"
...
def host_sample(proc: Path = Path("/proc")) -> dict:
    memory, vm = numeric_fields(proc / "meminfo"), numeric_fields(proc / "vmstat")
    ...
    return {"available_bytes": memory["MemAvailable"], "swap_in_pages": vm["pswpin"],
            "swap_out_pages": vm["pswpout"], "psi_avg10": pressure}


def host_violation(sample: dict, baseline: dict, limits: Limits, *, admission=False) -> str | None:
    ...
    if any(sample[key] != baseline[key] for key in ("swap_in_pages", "swap_out_pages")):
        return "host_swap_activity"
...
def monitor(child: subprocess.Popen, cgroup: Path, limits: Limits, baseline: dict, receipt: dict) -> None:
    ...
    while True:
        ...
        reason = host_violation(host, baseline, limits)
...
def supervise(command: list[str], output: Path, limits: Limits) -> dict:
    ...
    try:
        ...
        receipt.update(cgroup=str(cgroup), baseline=baseline, minimum_available_bytes=baseline["available_bytes"],
