"""Create-once, verified SQLite v11 storage for one canonical CDR observation."""
...
def _historical_evidence(document: Any, label: str) -> str:
    value = _mapping(document, f"{label} document").get("historical_evidence_id")
    if not isinstance(value, str) or not SHA256.fullmatch(value):
        _fail(f"{label} lacks verified historical evidence")
    return value
...
def _reject_overlapping_tiers(facts: Sequence[Mapping[str, Any]]) -> None:
    ...
    for row in facts:
        ...
        tier_marker = source_path.find(".tiers[")
        rate_parent = source_path[:tier_marker] if tier_marker >= 0 else None
        key = (
            row["product_uid"],
            row["canonical_key"],
            rate_parent or document.get("source_pattern"),
...
def _normalize_projections(value: Mapping[str, Any], accounting: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    for row in output["product_facts"]:
        ...
        if row["value_type"] == "rate" and number is not None and not 0 <= number <= 1:
    ...
    for row in output["product_changes"]:
        ...
        current = products.get(expected["product_uid"])
        historical = current is None
        if (
            expected["event_id"] in event_ids
            or not PROVIDER_UID.fullmatch(expected["provider_uid"])
            or not PRODUCT_UID.fullmatch(expected["product_uid"])
            or historical and expected["event_type"] not in {"product_removed", "fact_removed"}
            or not historical and current["provider_uid"] != expected["provider_uid"]
            or expected["event_type"] == "product_removed" and not historical
        ):
            _fail("change identity or provider is invalid")
        historical_evidence = None
        if _mapping(row["document"], "change document").get("historical_evidence_id") is not None:
            historical_evidence = _historical_evidence(row["document"], "change")
        if historical and historical_evidence is None:
            _fail("removed product lacks verified historical evidence")
        evidence = (
            historical_evidence
            if historical
            else _bound_evidence(
                row["document"],
                frozenset({evidence_by_uid[expected["product_uid"]]}),
                "change",
            )
        ...
        if historical_evidence is not None:
            expected_document["historical_evidence_id"] = historical_evidence
        document = _document(
...
def _verify_connection(connection: sqlite3.Connection, path: Path, expected_sidecar_bytes: bytes | None, expected_projections: Mapping[str, Any] | None, expected_normalization_version: str | None, expected_generated_at: str | None) -> DatabaseVerification:
    ...
    if schema_sha != expected_schema_fingerprint():
        _fail(f"SQLite tables, constraints, indexes, or triggers differ from v{SCHEMA_VERSION}")
...
# Imported after the validation/readback layer is defined. The writer calls this
# module's validators, while this stable public import keeps existing callers simple.
from cdr_observation_db_build import build_observation_database  # noqa: E402
