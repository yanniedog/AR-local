...
from cdr_terms.executable_v3_contract import validate_asset,CAPABILITY
from cdr_terms.executable_v3_sources import validate_destination,_json as source_json
...
def package_executable_v3(snapshot,*,core,details,core_asset_sha256,details_asset_sha256,run_date,write_asset):
    ...
    for capability,products in sorted(snapshot.items()):
        ...
        shards,index,group={},{},{}
        def document(values):
            return dict(schema_version=3,capability=capability,run_date=run_date,core_asset_sha256=core_asset_sha256,details_asset_sha256=details_asset_sha256,products=values)
        ...
        def flush():
            ...
            name=f'monetary_v3_{capability}_shard_{len(shards):03d}'
        ...
        for key,asset in sorted(products.items()):
            ...
            group[key]=asset
        flush()
        if index:routes[capability]={'index':write(f'monetary_v3_{capability}_index',document(index)),'shards':shards}
    return {'schema_version':3,'capabilities':routes} if routes else None
