...
def _coverage(export_root: Path) -> tuple[str, dict[str, Any], list[dict[str, Any]]]:
    ...
    provenance_complete = (
        status.get("failure_provenance_complete") is True and providers_reconcile
    )
    incomplete = (
    ...
    return observation_state, coverage, provider_states
...
def verify_completion_marker(marker: Mapping[str, Any], state_dir: Path, date: str) -> bool:
    try:
        ...
        if manifest_banks_rate_count(marker) <= 0:
            return False
        contract_path = state_dir.expanduser().resolve() / str(marker["export_contract_path"])
        contract = load_contract(contract_path)
