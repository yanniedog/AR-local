...
if (!profile) {
  fail('sync-firebase-eas-env: EAS_PROFILE is required');
}
...
function runEas(args, label) {
  ...
  if (result.status !== 0) {
    fail(`sync-firebase-eas-env: ${label} failed (exit ${result.status ?? 1})`, result.status ?? 1);
  }
}
...
function syncEnv(name, type, value, filePath) {
  ...
  if (!combined.includes(`already has an environment variable named "${name}"`)) {
    ...
    fail(`sync-firebase-eas-env: create ${name} failed`, create.status ?? 1);
  }
...
function main() {
  ...
  const plistRaw = process.env.GOOGLE_SERVICE_INFO_PLIST?.replace(/^\uFEFF/, '').trim();
  if (plistRaw) {
    if (!plistRaw.startsWith('<')) {
      fail('sync-firebase-eas-env: GOOGLE_SERVICE_INFO_PLIST must start with "<" (raw plist XML)');
    }
...
try {
  ...
  const code = err && typeof err === 'object' && 'exitCode' in err ? Number(err.exitCode) : 1;
