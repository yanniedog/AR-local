...
from .executable_v4_contract import CAPABILITY
...
def material_projection(subject):
    return {key:subject[key] for key in ('scope','routing','policy','authorityGraph','evidence','termRevisionIds','documentVersionIds')}
...
def _positive(store,subject,evidence_sha,previous):
    ...
    if (not isinstance(evidence,dict) or set(evidence)!=POSITIVE or evidence['schemaVersion']!=4 or evidence['passed'] is not True
    ...
    latest=store.db.execute('SELECT subject_id FROM executable_subjects_v4 WHERE scope_id=? ORDER BY sequence DESC LIMIT 1',(subject['scopeId'],)).fetchone()
    ...
    from .executable_v4_benchmarks import verify_benchmark
...
def review_subject(store,subject_id,*,decision,reviewer,reviewer_kind,reviewed_at,evidence_sha256,reason,expected_previous_review_id):
    ...
    with store.db:
        ...
        row=store.db.execute('SELECT * FROM executable_subjects_v4 WHERE subject_id=?',(subject_id,)).fetchone()
        ...
        previous=store.db.execute('SELECT * FROM executable_reviews_v4 WHERE subject_id=? ORDER BY sequence DESC LIMIT 1',(subject_id,)).fetchone()
        ...
        if decision=='approved':
            ...
            if proof!={'schemaVersion':4,'subjectId':subject_id,'decision':decision,'previousReviewId':expected_previous_review_id,'reason':reason}:
        ...
        store.db.execute('INSERT INTO executable_reviews_v4(review_id,subject_id,previous_review_id,decision,reviewer,reviewer_kind,reviewed_at,evidence_sha256,source_snapshot_sha256,authority_graph_sha256,benchmark_sha256,reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(identity,*fields))
    return identity


@evidence_checked
def current_approval(store,subject):
    row=store.db.execute('SELECT * FROM executable_reviews_v4 WHERE subject_id=? ORDER BY sequence DESC LIMIT 1',(subject['id'],)).fetchone()
