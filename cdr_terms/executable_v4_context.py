...
from .executable_v4_contract import CAPABILITY,schema_validate,validate_asset
from .executable_v4_sources import validate_destination
...
def target_binding(target,subject,core):
    ...
    if target['productKey']!=subject['scope']['productKey'] or target['productRecordSha256']!=subject['routing']['productRecordSha256']:
        raise ValueError('Activity product publication is not verified')
    if target['kind']=='product':return
    if target['kind']!='rate_variant' or target['section']!='Savings':raise ValueError('Activity product publication is not verified')
    rows=core.get('sections',{}).get('Savings',{}).get('rates',[]);index=target['coreRowIndex']
    if type(index) is not int or not 0<=index<len(rows):raise ValueError('Activity product publication is not verified')
    row=rows[index]
    if row.get('product_key')!=target['productKey'] or type(row.get('rate_index')) is not type(target['rateIndex']) or row.get('rate_index')!=target['rateIndex'] or digest(row)!=target['rowSha256']:raise ValueError('Activity product publication is not verified')
...
def context_binding(context,subject,operation):
    ...
    route=manifest['executable_v4']['capabilities'][capability]
