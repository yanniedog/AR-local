...
from .executable_v4_contract import validate_asset,CAPABILITY
from .executable_v4_reviews import current_approval
from .executable_v3_evidence import evidence_checked


@evidence_checked
def build_asset(store,product_key,*,routing,capability=CAPABILITY):
    if capability != CAPABILITY:raise ValueError("Unsupported activity capability")
    ...
    rows=store.db.execute('SELECT s.* FROM executable_subjects_v4 s JOIN executable_scopes_v4 k USING(scope_id) '
        'WHERE k.product_key=? AND s.capability=? AND s.observation_id=? AND s.sequence=(SELECT MAX(x.sequence) FROM executable_subjects_v4 x WHERE x.scope_id=s.scope_id) '
    ...
    value=dict(schemaVersion=4,capability=capability,productKey=product_key,routing=routing,approvalPolicy='as_of_adopted_edition',subjects=subjects)
...
def publish_asset(store,product_key,*,routing,expected_previous_publication_id,expected_observation_id,published_at,capability=CAPABILITY):
    ...
    with store.db:
        ...
        previous=store.db.execute('SELECT publication_id FROM executable_publications_v4 WHERE product_key=? AND capability=? ORDER BY sequence DESC LIMIT 1',(product_key,capability)).fetchone()
        ...
        identity=asset['identitySha256'] if asset else digest(dict(schemaVersion=4,capability=capability,productKey=product_key,observationId=expected_observation_id,state='removed'))
        ...
        store.db.execute('INSERT INTO executable_publications_v4(publication_id,product_key,capability,observation_id,previous_publication_id,state,identity_sha256,published_at,payload_json) VALUES(?,?,?,?,?,?,?,?,?)',(publication_id,*fields))
