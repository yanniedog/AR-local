...
Durable notes for cloud agents running in this VM. The startup update script only refreshes Python deps (`pip install --user -r requirements.txt` + `cryptography pytest`); everything below is already provisioned in the VM image (persisted from setup), so do not re-run one-off setup unless it is missing.

- **Interpreters:** scripts and `package.json` invoke bare `python`/`node`. The base image only ships `python3` and an old `/exec-daemon/node` (v22). Setup added shims so login shells resolve the right versions: `python -> python3` (also `/usr/local/bin/python`), and Node 24 via nvm exposed through `~/.local/bin/node|npm|npx` (which precede `/exec-daemon` on the login `PATH`). If `node --version` shows v22 in a non-login shell, run via `bash -lc '...'` or `nvm use 24`. Python is 3.12 here (repo targets 3.10+); pipeline deps are stdlib-only plus `tzdata`.
- **Dashboard needs external site assets:** `cdr_dashboard_server.py` hard-exits at startup unless it finds the AustralianRates `site` shell (`foundation.css` + `assets/banks/*`). Those live in a separate public repo (`yanniedog/AustralianRates`), sparse-cloned to `~/australianrates/site` during setup. Run with `--site-root "$HOME/australianrates/site"`. To refresh/recreate: `git clone --depth 1 --filter=blob:none --sparse https://github.com/yanniedog/AustralianRates.git ~/australianrates && git -C ~/australianrates sparse-checkout set site`.
- **No committed data:** `runs/` is gitignored and empty on a fresh VM. The dashboard requires generated exports first. Real-data path (no mocks, per repo policy): `python cdr_full_ingest.py --holders "<brand>" --max-products 30 --resume` (needs outbound access to `api.cdr.gov.au`; note brands are e.g. `ANZ`, `CommBank`, `Bendigo Bank`, `Macquarie` — not "Commonwealth Bank"), then `python cdr_outputs.py runs/<UTC-date>` to build `_exports/`. `python cdr_daily.py --workers 8` does ingest+build in one shot but ingests all ~117 holders.
- **Run + verify (local dev):** `python cdr_dashboard_server.py --exports runs/<date>/_exports --host 127.0.0.1 --port 8808 --site-root "$HOME/australianrates/site"`, then `python verify_local.py --base-url=http://127.0.0.1:8808/` (exit 0 = OK). Data APIs need a date, e.g. `/api/banks/section?section=Savings&date=<date>`. NOTE: AGENTS ship-bar/`verify:pi` steps target the Pi at `100.78.28.10`, which is unreachable from this cloud VM — use `127.0.0.1` here for local dev/verify.
- **Tests:** Python `python -m pytest tests/ -q` (no config file; needs `pytest`). Mobile app (`mobile/`, Expo SDK 54, Node 24): `npm ci` then `npm run typecheck` / `npm run lint` / `npm run test:scripts`. There is no Python linter configured.
- **Mobile app cannot run end-to-end here** (Expo/React Native needs a device/emulator); validate it via the automated checks above.
- **Node workflow tooling** (`npm run chief:scan`, `wait-for-bots`, `pr:*`, etc.) needs Node 24 + `gh`; it is dev/PR plumbing, not part of the product runtime.
